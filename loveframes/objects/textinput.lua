...
function newobject:update(dt)
	...
	if multiline then
		...
		if itemheight > height then
			if not vbar then
				local scrollbody = loveframes.objects["scrollbody"](self, "vertical")
		...
		if itemwidth > width then
			if not hbar then
				local scrollbody = loveframes.objects["scrollbody"](self, "horizontal")
				scrollbody.internals[1].internals[1].autoscroll = self.autoscroll
				table.insert(self.internals, scrollbody)
				self.hbar = true
				if self.vbar then
		...
		if linenumbers then
			if not self.linenumberspanel then
				local linenumberspanel = loveframes.objects["linenumberspanel"](self)
